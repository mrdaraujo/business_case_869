...
def DataMerging(csv_left, csv_stores):
    ...
    df = df.drop(columns=['date', 'id', 'onpromotion', 'store_nbr', 'family'])
    df = df.groupby(["year","month","city"]).sum().reset_index()
...
def Preproc():
    ...
    preprocessor = ColumnTransformer([
            ...
            ('cat_tr', cat_transformer, ['year', 'month', 'city'])
...
def Pipe(X_train, y_train):
    ...
    joblib.dump(pipeline, 'model_city.joblib')
